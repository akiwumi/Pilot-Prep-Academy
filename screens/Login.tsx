    ...
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden bg-background">
        ...
        <div className="relative w-full h-[320px] bg-background overflow-hidden rounded-b-[2.5rem] shadow-2xl z-0">
            ...
            <div className="absolute bottom-0 left-0 w-full px-6 pb-12 flex flex-col justify-end h-full">
                ...
                <p className="text-text-secondary mt-2 text-sm font-light">Your journey to the skies begins here.</p>
            </div>
        </div>
        ...
        <div className="flex-1 flex flex-col px-6 -mt-6 z-10 relative pb-8">
            {/* Toggle */}
            <div className="bg-surface p-1.5 rounded-xl shadow-lg border border-slate-200 flex mb-8">
                <label className="flex-1 cursor-pointer">
                    <input type="radio" name="auth-mode" value="login" className="peer sr-only" defaultChecked />
                    <div className="w-full py-3 rounded-lg text-sm font-medium text-center transition-all duration-300 peer-checked:bg-primary peer-checked:text-white peer-checked:shadow-md text-text-secondary hover:text-slate-900">
                        Login
                    </div>
                </label>
                <label className="flex-1 cursor-pointer">
            ...
            <form className="flex flex-col gap-5 w-full max-w-md mx-auto" onSubmit={(e) => { e.preventDefault(); onNavigate(Screen.CourseOverview); }}>
                <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 ml-1">Email / Username</label>
                    <div className="relative group">
                        ...
                        <input type="email" className="w-full h-14 bg-surface text-slate-900 placeholder:text-text-secondary border border-slate-200 rounded-xl pl-11 pr-4 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all shadow-sm" placeholder="pilot@example.com" />
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative group">
                        ...
                        <input type="password" className="w-full h-14 bg-surface text-slate-900 placeholder:text-text-secondary border border-slate-200 rounded-xl pl-11 pr-12 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all shadow-sm" placeholder="••••••••" />
                        <button type="button" className="absolute right-0 top-0 bottom-0 w-12 flex items-center justify-center text-slate-500 hover:text-primary transition-colors">
            ...
            <div className="grid grid-cols-2 gap-4 w-full max-w-md mx-auto mb-4">
                <button className="flex items-center justify-center gap-3 bg-surface border border-slate-200 hover:bg-surface-highlight text-slate-900 py-3.5 rounded-xl transition-all shadow-sm">
