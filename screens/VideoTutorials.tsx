  ...
  return (
    <div className="bg-background text-slate-900 font-display overflow-x-hidden pb-24">
        <div className="flex items-center px-4 py-3 justify-between sticky top-0 z-50 bg-background/95 backdrop-blur-md">
            <button onClick={() => onNavigate(Screen.LearningLibrary)} className="text-slate-700 flex size-12 shrink-0 items-center justify-start cursor-pointer">
                <span className="material-symbols-outlined text-[24px]">arrow_back</span>
            </button>
            <h2 className="text-slate-900 text-lg font-bold leading-tight tracking-[-0.015em] flex-1 text-center">Video Tutorials</h2>
            <div className="flex w-12 items-center justify-end">
                <button className="flex size-12 cursor-pointer items-center justify-end overflow-hidden rounded-lg bg-transparent text-slate-700 gap-2 text-base font-bold leading-normal tracking-[0.015em] min-w-0 p-0">
        ...
        <div className="px-4 py-2">
            <div className="flex w-full flex-1 items-stretch rounded-xl h-12 bg-surface shadow-sm">
                ...
                <input className="flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-xl text-slate-900 focus:outline-0 focus:ring-0 border-none bg-transparent focus:border-none h-full placeholder:text-text-secondary px-2 text-base font-normal leading-normal" placeholder="Search maneuvers, theory..." />
            </div>
        </div>
        ...
        <div className="flex w-full overflow-x-auto no-scrollbar px-4 py-3 gap-2">
            ...
            {['Pre-flight', 'Maneuvers', 'Meteorology', 'Comms'].map((topic) => (
                <button key={topic} className="flex h-9 items-center justify-center rounded-full bg-surface border border-slate-200 px-4 text-sm font-medium text-slate-900 shadow-sm shrink-0 whitespace-nowrap">
                    {topic}
                </button>
            ))}
        </div>
        ...
        <div className="flex flex-col pb-2">
            <h2 className="text-slate-900 tracking-tight text-[22px] font-bold leading-tight px-4 pb-3 pt-4">Continue Learning</h2>
            <div className="flex w-full overflow-x-auto no-scrollbar px-4 pb-4 gap-4">
                {[
                    ...
                    <div key={i} className="flex flex-col gap-2 w-60 shrink-0 cursor-pointer group">
                        ...
                        <div>
                            <h3 className="text-slate-900 text-base font-semibold leading-tight truncate">{card.title}</h3>
                            <p className="text-text-secondary text-xs font-normal mt-1">{card.sub}</p>
                        </div>
                    </div>
                ))}
            </div>
        ...
        <div className="flex flex-col">
            <div className="flex items-center justify-between px-4 pb-3 pt-2">
                <h2 className="text-slate-900 tracking-tight text-[22px] font-bold leading-tight">Recommended for You</h2>
            ...
            <div className="flex flex-col px-4 gap-4">
                {[
                    ...
                    <div key={i} className="flex gap-4 items-start bg-surface p-3 rounded-xl shadow-sm hover:bg-surface-highlight transition-colors cursor-pointer border border-slate-200">
                        <div className="relative w-32 shrink-0 aspect-video rounded-lg overflow-hidden bg-surface-highlight">
                        ...
                        <div className="flex flex-col flex-1 min-w-0 gap-1">
                            <div className="flex justify-between items-start">
                                <h3 className="text-slate-900 text-sm font-bold leading-tight line-clamp-2">{item.title}</h3>
                            ...
                            <div className="flex items-center gap-3 mt-auto pt-2">
                                <div className={`flex items-center gap-1 ${item.downloaded ? 'text-primary' : 'text-text-secondary'}`}>
