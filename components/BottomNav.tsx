...
export const BottomNav: React.FC<BottomNavProps> = ({ activeScreen, onNavigate, variant }) => {
  const getIconClass = (screen: Screen) => {
    return activeScreen === screen ? 'text-primary' : 'text-text-secondary hover:text-white transition-colors';
  };
  ...
  if (variant === 'directory') {
    return (
      <div className="fixed bottom-0 w-full max-w-md bg-surface border-t border-slate-800 pb-safe pt-2 px-2 z-40 flex justify-between h-[80px]">
        ...
        <button className="flex flex-1 flex-col items-center justify-center gap-1 p-2 text-text-secondary">
